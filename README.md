...
| **StepAudio 实时播报** | 使用 stepaudio-2.5-realtime 模型低延迟语音播报结果 |
...
1. 打开设置 → 切换到 **语音播报** 标签页
2. 勾选 **启用 StepAudio 实时语音播报**
3. 选择播报音色（青年男声/青年女声/磁性男声/温柔女声）
4. 保存设置后，每次语音识别结果粘贴发送时，会自动播报文本内容

**为什么选择 stepaudio-2.5-realtime 而非传统 TTS？**
- 响应延迟更低，语音更自然有表现力
- 适用于 VibeCoding 决策播报和好友聊天语音回复场景
- 端到端模型，无需中间文本处理步骤
